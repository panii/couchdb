defmodule ReplicationTest do
  ...
  # TODO: Parameterize the host name for these pairs
  @db_pairs_prefixes [
    {"local-to-local", "", ""},
  ...
  def run_simple_repl(src_prefix, tgt_prefix) do
    ...
    result = replicate(src_db_name, tgt_db_name)
    assert result["ok"]
  ...
  def replicate(src, tgt, options \\ []) do
    ...
    resp = Couch.post("/_replicate", headers: headers, body: body)
  ...
  def add_attachment(db_name, doc, att \\ []) do
    ...
    att = Keywords.merge(defaults, att) |> Enum.into(%{})
    uri = "/#{db_name}/#{URI.encode(doc[:_id])}/#{att[:name]}"
    headers = %{"Content-Type" => att[:content_type]}
    params = if doc[:_rev], do: %{:rev => doc[:_rev]}, else: %{}
...
#
#   def att2_data do
#     File.read!("test/data/lorem_64.txt")
#   end
# end
